...
def _quiet_fake_which(program):
    return struct(_coevent_fake_which = program)
...
def _find_cxx(repository_ctx):
    ...
    if hasattr(real_cxx, "_coevent_fake_which"):
        real_cxx = repository_ctx.which(real_cxx._coevent_fake_which)
        if real_cxx == None:
            find_cc(struct(
...
def _build_coevent_cc_wrapper(repository_ctx):
    ...
    repository_ctx.template("extra_tools/coevent_cc_wrapper", repository_ctx.attr._coevent_cc_wrapper, {
        "{COEVENT_REAL_CC}": repr(str(real_cc)),
        "{COEVENT_REAL_CXX}": repr(str(real_cxx)),
    })
    return repository_ctx.path("extra_tools/coevent_cc_wrapper")

def _needs_coevent_cc_wrapper(repository_ctx):
...
def cc_autoconf_impl(repository_ctx):
    overriden_tools = {}
    if _needs_coevent_cc_wrapper(repository_ctx):
        # Bazel uses "gcc" as a generic name for all C and C++ compilers.
        overriden_tools["gcc"] = _build_coevent_cc_wrapper(repository_ctx)
    return _upstream_cc_autoconf_impl(repository_ctx, overriden_tools = overriden_tools)

cc_autoconf = repository_rule(
    implementation = cc_autoconf_impl,
    attrs = {
        "_coevent_cc_wrapper": attr.label(default = "@com_github_rnburn_coevent//bazel:cc_wrapper.py"),
    },
    environ = [
        "ABI_LIBC_VERSION",
        "ABI_VERSION",
